...
void handleWasmFileUpload()
{
    ...
    case UPLOAD_FILE_ABORTED:
        ...
        Serial.println(createErrorMessage("file write failed"));
        server.send(
            500,
            "application/json",
            createErrorMessage("file write failed"));
        Serial.println("Failure response sent to client");
        return;
...
{
    liveUpdate = (server.hasArg("liveUpdate") &&
                  server.arg("liveUpdate") == "true");
    reservedStackSize = server.hasArg("reservedStackSize") ? server.arg("reservedStackSize").toInt() : 4096;
    reservedInitialMemory = server.hasArg("reservedInitialMemory") ? server.arg("reservedInitialMemory").toInt() : 65556;
    memoryLimit = server.hasArg("memoryLimit") ? server.arg("memoryLimit").toInt() : 65556;
}
...
void configServerRoutes()
{
    ...
    server.on("/task/ ", HTTP_POST, handleWasmFileEdit);
